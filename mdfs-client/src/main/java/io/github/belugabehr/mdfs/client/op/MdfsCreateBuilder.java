...
public class MdfsCreateBuilder implements CreateBuilder {
	...
	public void copyFromFile(final Path source) throws IOException {
		...
		final Entry<String, String> namespaceAndId = MdfsPaths.parse(target);

		final MFile.Builder mFile = MFile.newBuilder()
				.setFileId(MFileId.newBuilder().setNamespace(namespaceAndId.getKey())
						.setId(ByteString.copyFromUtf8(namespaceAndId.getValue())))
				.setFileSize(fileSize).setBlockSize(blockSize);
		...
		for (String dataNode : locations) {
			final URI node = URI.create(dataNode);
